...
if (plot.all.together) {
  ...
  #### hpi 1
  samples <- unique(metafilt[metafilt$Time == 1 & !grepl('CAGE', metafilt$group), combine.groups])
  FigY    <- plotfun.all(samples)
  
  fig.width  <- (visto - visfrom) / fig.w.size
  fig.height <- fig.width * fig.h2w.ratio # max(length(unique(samples))*3 + 3, 12)
  
  ggsave(paste0(fig.dir, '/', virus, '_', prime, '_', '1h', '_', file_name_suffix, '.jpg'), plot = FigY, height = fig.height, width = fig.width, limitsize = F)
  
  #### hpi 2
  samples <- unique(metafilt[metafilt$Time == 2 & !grepl('CAGE', metafilt$group), combine.groups])
  FigY    <- plotfun.all(samples)
  ...
  #### hpi 4
  samples <- unique(metafilt[metafilt$Time == 4 & !grepl('CAGE', metafilt$group), combine.groups])
  FigY    <- plotfun.all(samples)
  ...
  #### hpi 6
  samples <- unique(metafilt[metafilt$Time == 6 & !grepl('CAGE', metafilt$group), combine.groups])
  FigY    <- plotfun.all(samples)
  ...
  #### hpi 8
  samples <- unique(metafilt[metafilt$Time == 8 & !grepl('CAGE', metafilt$group), combine.groups])
  FigY    <- plotfun.all(samples)
  ...
  #### hpi 12
  samples <- unique(metafilt[metafilt$Time == 12 & !grepl('CAGE', metafilt$group), combine.groups])
  FigY    <- plotfun.all(samples)
  ...
  #### hpi 24
  samples <- unique(metafilt[metafilt$Time == 24 & !grepl('CAGE', metafilt$group), combine.groups])
  FigY    <- plotfun.all(samples)
  ...
  #### hpi 48
  samples <- unique(metafilt[metafilt$Time == 48 & !grepl('CAGE', metafilt$group), combine.groups])
  FigY    <- plotfun.all(samples)
