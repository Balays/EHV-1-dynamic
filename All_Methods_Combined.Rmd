...
sig.cluster.sum <- orf.perc.sum[,.(sum_read_count=sum(read_count), mean_read_count=mean(read_count), sd_read_count=sd(read_count)), 
                              by=.(EndType, labels, hpi, Time, cell_line, seqnames, norm_base, sample, group, norm.method)]
...
all.cluster.sum <- orf.perc.sum[,.(sum_read_count=sum(read_count), mean_read_count=mean(read_count), sd_read_count=sd(read_count)), 
                              by=.(EndType, best_label, hpi, Time, cell_line, seqnames, norm_base, sample, group, norm.method)]
...
```{r, fig.show='hold',  fig.width = figw, fig.height = figh, eval=T}
ggp <- 
      ggplot(sig.cluster.sum[norm.method %in% normMethods],
      ...
      # theme(strip.text.y = element_blank()) +
      #ggtitle(title) +
      facet_nested(rows=vars(EndType, norm.method), cols=vars(labels), scales = "free_y") ## cols=vars(norm.method)
...
plotlist <- purrr::map(1:length(unique(na.omit(DT$labels))), mapfun, unique(na.omit(DT$labels)), 'labels', c("viral_read.count", "host_read.count"), DT)
...
ggp <- 
      ggplot(all.cluster.sum[norm.method %in% normMethods],
      ...
      # theme(strip.text.y = element_blank()) +
      #ggtitle(title) +
      facet_nested(rows=vars(EndType, norm.method), cols=vars(best_label), scales = "free_y") ## cols=vars(norm.method)
...
plotlist <- purrr::map(1:length(unique(DT$best_label)), mapfun, unique(DT$best_label), 'best_label', c("viral_read.count", "host_read.count"), DT)
...
setDT(orf.perc.mean)
class.dt <- unique(orf.perc.mean[, .(EndType, norm.method, gene, Kinetic_class, best_label, labels)])
class.dt <- class.dt[, .N, by=.(EndType, norm.method, Kinetic_class, best_label, labels)]
...
ggplot(class.dt[norm.method %in% normMethods],
  ...
  #scale_y_continuous(labels = scales::percent_format()) +
  theme_ipsum() +
  facet_nested(cols=vars(labels), rows=vars(EndType, norm.method))
