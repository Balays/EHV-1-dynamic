---
title: "EHV-1 Gene dynamics (TSS ~ TES)"
author: Balázs Kakuk
output: 
  html_document:
    theme: cosmo
...
if (EndType == 'TES') {
  ...
  message('Keeping those where both the TSS and TES were correct !')
...
class.dt      <- cluster_dt[   , .N, by=.(Kinetic_class, cluster )]
...
p <- ggplot(orf.perc.mean.6h_norm.melt, aes(x = hpi, y = gene, fill = delta6_mean)) +
  ...
  scale_fill_gradient2(#low = "#d73027", mid = "white", high = "#4575b4",
    low = "#d73027", mid = "white", high = "#4575b4",
    midpoint = median(orf.perc.mean.6h_norm.melt$delta6_mean, na.rm = TRUE), 
...
# Calculate z-scores
z_scores_data <- calculate_z_scores(data=orf.perc.mean.cast, time_points)
#z_scores_data[,z_6h := 0]


# Identify numeric columns
numeric_cols <- names(orf.perc.mean.cast)[sapply(orf.perc.mean.cast, is.numeric)]

# Calculate z-scores for numeric columns
dt_z <- copy(orf.perc.mean.cast)  # Create a copy to store z-scores
dt_z[, (numeric_cols) := lapply(.SD, function(x) (x - mean(x, na.rm = TRUE)) / sd(x, na.rm = TRUE)), .SDcols = numeric_cols]
colnames(dt_z)
z_scores_data   <- dt_z
...
dt <- orf.perc.mean.cast
dt_row_normalized <- copy(dt)
dt_row_normalized[, (numeric_cols) := lapply(.SD, function(x) (x - mean(x, na.rm = TRUE)) / sd(x, na.rm = TRUE)), .SDcols = numeric_cols]

# Normalize across columns
dt_col_normalized <- copy(dt_row_normalized)
dt_col_normalized[, (numeric_cols) := lapply(.SD, function(x) (x - mean(x, na.rm = TRUE)) / sd(x, na.rm = TRUE)), .SDcols = numeric_cols]


melted_z_scores <- melt(dt_col_normalized, variable.name = 'hpi', id.vars = c('gene', 'Kinetic_class', 'cluster'))
melted_z_scores[,hpi := factor(hpi, levels = time_points)]
...
p <- ggplot(melted_z_scores, aes(x = hpi, y = gene, fill = value)) +
  ...
  scale_fill_gradient2(#low = "#d73027", mid = "white", high = "#4575b4",
    low = "#d73027", mid = "white", high = "#4575b4",
    midpoint = median(melted_z_scores$value, na.rm = TRUE), 
...
### Literature clusters
```{r, fig.show='hold',  fig.width = 12, fig.height = 8, eval=T}
