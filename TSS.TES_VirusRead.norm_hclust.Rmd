---
title: "EHV-1 Canonic Transcript dynamics (TSS ~ TES)"
author: Balázs Kakuk
output: 
  html_document:
    theme: cosmo
...
# Canonic Transcript dynamics (TSS ~ TES)

## Those reads were counted only, where *both* the *3-prime* AND the *5-prime* adapters were "correct" (according to LoRTIA)

## When counting with *either* adapters, the clustering grouped ORF64 together with two late genes
...
samples_to_filt <- NULL # c('EHV-1_48h_3')  ## OR: NULL
...
kin.class.freq  <- data.table(cluster_dt)[,.N, by=.(Kinetic_class) ][order(Kinetic_class)]
```
...
plotfun <- function(DT, 
  ...
  if (nrow(DT) == 0) { 
    ...
    ggp <- 
      ...
      scale_y_continuous(labels = scales::percent_format(), name = 'Mean abundance %', n.breaks=4) +
...
ggsave('Figures/Figure 3.jpg',  ggp, width = figw*0.8, height = figh, limitsize = F)
...
ggp <- cowplot::plot_grid(
  ...
  cowplot::plot_grid(
    mapfun(1, orf.perc.mean, cluster.freq, palette = palette, legendpos='right', ncol=8),
    nrow = 1, rel_widths = c(1), align = 'vh', axis=c('tblr')
  ),
  
  cowplot::plot_grid(
    mapfun(2, orf.perc.mean, cluster.freq, palette = palette, legendpos='right', ncol=1),
    NULL, 
    ...
    NULL, NULL,
    nrow = 1, rel_widths = c(1, 1, 1, 1, 2, 1, 1), align = 'vh', axis=c('tblr')
  ),
  
  cowplot::plot_grid(
    mapfun(5, orf.perc.mean, cluster.freq, palette = palette, legendpos='right', ncol=8),
    nrow = 1, rel_widths = c(1), align = 'vh', axis=c('tblr')
  ),
  
  cowplot::plot_grid(
    mapfun(6, orf.perc.mean, cluster.freq, palette = palette, legendpos='right', ncol=8),
    nrow = 1, rel_widths = c(1), align = 'vh', axis=c('tblr')
  ),
  ...
  cowplot::plot_grid(
    ...
    NULL, 
    mapfun(11, orf.perc.mean, cluster.freq, palette = palette, legendpos='right', ncol=3),
    NULL, 
    nrow = 1, rel_widths = c(3, 1, 3, 1), align = 'vh', axis=c('tblr')
  ),
  
  cowplot::plot_grid(
    mapfun(8, orf.perc.mean, cluster.freq, palette = palette, legendpos='right', ncol=8),
    nrow = 1, rel_widths = c(1), align = 'vh', axis=c('tblr')
  ),
  
  cowplot::plot_grid(
    mapfun(9, orf.perc.mean, cluster.freq, palette = palette, legendpos='right', ncol=8),
    nrow = 1, rel_widths = c(1), align = 'vh', axis=c('tblr')
  ),
  
  cowplot::plot_grid(
    mapfun(10, orf.perc.mean, cluster.freq, palette = palette, legendpos='right', ncol=4),
    mapfun(12, orf.perc.mean, cluster.freq, palette = palette, legendpos='right', ncol=3),
    ggl_legend, ## Heres the color coding ~ aka legend
    nrow = 1, rel_widths = c(4, 3, 1), align = 'vh', axis=c('tblr')
  ),
  
  rel_heights = c(2,1,2,1,1,2,2,1), ncol = 1, align = 'vh', axis=c('tblr')

)

ggsave('Figures/Figure 5.jpg',  ggp, width = figw*0.8, height = figh, limitsize = F)

ggsave(paste0(fig.dir, '/', EndType, '_', norm.method, '_deNovo.clusters', '_mean.linear.jpg'), ggp, width = figw, height = figh, limitsize = F)
...
ggsave('Figures/Figure 5 TSS-TES deNovo Clusters REVIEW.jpg',  ggp, width = figw*0.8, height = figh, limitsize = F)
...
```{r, fig.show='hold',  fig.width = 15, fig.height = 16, eval=T}
ggplot(cluster_dt,
...
ggp <- ggplot(cluster_dt,
       aes(x=cluster, y=gene, fill=Kinetic_class)) +
  geom_tile() +
  # coord_flip() +
  scale_fill_manual(values = my_pal) +
  #scale_y_discrete(breaks=NULL) +
  ...
  theme(legend.position = 'bottom',
        ...
        axis.text.x = element_text(face='italic')) # + facet_wrap(~cluster, ncol = 10)

ggsave('Figures/Supp Fig S8 GENE Kinetic Classes and denovo clusters.jpg',  ggp, width = 13, height = 16, limitsize = F)
...
```{r, fig.show='hold',  fig.width = 6, fig.height = 9, eval=T}
clusters_to_plot_in_group <- cluster.freq[N >= 4, cluster]

gg <- ggplot(DT[cluster %in% clusters_to_plot_in_group],
             aes(x=Time, y=mean)) +
    geom_line(aes(group=rep_gene, color=as.factor(cluster))) +
    #geom_smooth(aes(group=rep_gene, color=as.factor(cluster))) +
    #ggtitle(cluster_toplot) + 
    scale_color_manual(values = palette[]) +
    guides(color=guide_legend("de-novo cluster")) +
...
ggsave(paste0(fig.dir, '/', EndType, '_', norm.method, '_Combined.DeNovo.Clusters.', '_mean.lines.jpg'), ggp, width = figw, height = figh, limitsize = F)

```


```{r, fig.show='hold',  fig.width = 6, fig.height = 9, eval=T}
DTuni <- unique(DT[,.(gene, cluster, mean, sd, Time)])
clusters_to_plot_in_group <- cluster.freq[N >= 4, cluster]

gg <- ggplot(DTuni[cluster %in% clusters_to_plot_in_group],
             aes(x=Time, y=mean)) +
  geom_line( aes(group=gene, color=cluster)) +
  geom_pointrange(aes(group=gene, ymin=mean-sd, ymax=mean+sd), size = 0.2) +
  geom_point(aes(group=gene, fill=cluster), shape=21, size = 2) +
  #geom_smooth(aes(group=rep_gene, color=as.factor(cluster))) +
  #ggtitle(cluster_toplot) + 
  scale_fill_manual(values = palette[-c(2,3,4,7,10,11,12)]) +
  scale_color_manual(values = palette[-c(2,3,4,7,10,11,12)]) +
  scale_y_continuous(labels = scales::percent_format(), name = 'Mean abundance %', n.breaks=4) +
  scale_x_continuous(name = 'Hours past infection') +
  guides(color=guide_legend("de-novo cluster")) +
  #scale_x_continuous(breaks = c(1,2,4,6,8,12,24,48)) +
  ...
  theme(plot.margin = unit(c(1,5,1,1), 'mm'),
        ...
        panel.spacing = unit(2, 'mm'),
        #strip.background = element_rect(fill = alpha(palette[-c(2,3,4,7,10,11,12)], 0.4)),
        strip.text = element_text(face='italic', size=9)
  ) +
  facet_nested_wrap(~cluster, nrow = 3, scales = "free")

gg

ggsave(paste0('Figures/Figure 5 REVIEW 2.jpg'), gg, width = 6, height = 9, limitsize = F)

```


```{r, fig.show='hold',  fig.width = 18, fig.height = 12, eval=F}
ggm <- 
  mapfun(4, DTuni, cluster.freq, palette = palette, 
         geom1 = NULL,
         #geom1 = geom_point(aes(group=gene, fill=cluster), color='black', shape=21, size=3),
         geom2 = geom_line( aes(x=Time, y=mean, group=gene)),
         #geom3 = NULL,
         ncol=1)
...
```{r, fig.show='hold',  fig.width = 20, fig.height = 11, eval=T}

plot.axis.title.y <- c(7)
plot.axis.title.x <- c(12)
...
ggp <- cowplot::plot_grid(
  ...
  cowplot::plot_grid(
    mapfun(7,  orf.perc.mean, cluster.freq, palette = palette, legendpos='right', ncol=3),
    NULL, 
    mapfun(11, orf.perc.mean, cluster.freq, palette = palette, legendpos='right', ncol=2),
    NULL, 
    nrow = 1, rel_widths = c(3, 1, 2, 1), align = 'vh', axis=c('tblr')
  ),
  ...
  cowplot::plot_grid(
    ...
    mapfun(12, orf.perc.mean, cluster.freq, palette = palette, legendpos='right', ncol=3),
    #ggl_legend, ## Heres the color coding ~ aka legend
    nrow = 1, rel_widths = c(4, 3), align = 'vh', axis=c('tblr')
  ),
  
  rel_heights = c(1,1,1), ncol = 1, align = 'vh', axis=c('tblr')

)

ggsave('Figures/Supp Figure SX TSS-TES deNovo Clusters REVIEW 2.jpg',  ggp, 
       width = 20, height = 11, limitsize = F)
...
```{r, fig.show='hold',  fig.width = 6, fig.height = 6, eval=T}
DTuni <- unique(DT[,.(gene, Kinetic_class, mean, sd, Time)])

gg <- ggplot(DTuni[],
             aes(x=Time, y=mean)) +
  geom_line( aes(group=gene, color=Kinetic_class)) +
  geom_pointrange(aes(group=gene, ymin=mean-sd, ymax=mean+sd), size = 0.2) +
  geom_point(aes(group=gene, fill=Kinetic_class), shape=21, size = 2) +
  #geom_smooth(aes(group=rep_gene, color=as.factor(cluster))) +
  #ggtitle(cluster_toplot) + 
  scale_fill_manual(values = my_pal) +
  ...
  facet_nested_wrap(~Kinetic_class, nrow = 3, scales = "free")

gg

ggsave(paste0('Figures/Figure 3 REVIEW 2.jpg'), gg, width = 6, height = 6, limitsize = F)
