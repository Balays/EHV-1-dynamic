...
EndType <- 'Transcript.Isoform.Ratios.refgenes'
...
## summed read counts for each such CAGE TSS cluster
trids.cage.sum <- TR.gff.compare.uni.CAGE.ref.counts.melt.hpi[
  transcript_id %in% trids
  #& !is.na(CAGE_significance), 
  , .(sum_count=sum(count)), 
         by=.(seqnames, strand, CAGE_ID, CAGE.cluster.start, CAGE.cluster.end, CAGE_significance, support, score)]
...
trids.cage <- TR.gff.compare.uni.CAGE.ref.counts.melt.hpi[
  ...
  #& !is.na(CAGE_significance), 
  , .(sum_count=sum(count)), 
         by=.(seqnames, strand, prime3.TR, CAGE_ID, CAGE.cluster.start, CAGE.cluster.end, CAGE_significance, support, score)]
...
ref.trs  <- unique(viral.mrna[type == 'transcript', .(seqnames, strand, transcript_id, prime3.TR, prime5.TR)])
## unique 3-primes
ref.trs[,TES := .GRP, prime3.TR]
ref.TESs <- unique(ref.trs[,.(seqnames, strand, prime3.TR, TES)])

## add windows for merging
ref.TESs  [,':='(start = prime3.TR-10, end=prime3.TR+10)]
trids.cage[,':='(start = prime3.TR-10, end=prime3.TR+10)]

trids.cage[,TX_ID := .GRP, by=.(prime3.TR, CAGE_ID)]

foo <- foverlaps2(trids.cage, ref.TESs, by = c('seqnames', 'strand', 'start', 'end'), minoverlap = 1)
...
TX_nov <- foo.filt[,.(sum_TES_count=sum(sum_count)), by=.(seqnames, strand, Ref.prime3.TR, TES, CAGE_ID, CAGE.cluster.start, CAGE.cluster.end, CAGE_significance, support, score)]
...
```{r, fig.show='hold',  fig.width = 10, fig.height = 5, eval=T}
mapfun('ORF9',  orf.sum.perc, legend.position='right',  rel_widths=c(1, 1), title = 'ORF9')
ggsave('../EHV-1 dynamic article/Review 2024 nov/ORF9.jpg', height = 5, width = 10)

gene_to_plot <- 'NOIR'
mapfun(gene_to_plot,  orf.sum.perc, 
       visfrom = min(TR.merged.data.gene[gene == gene_to_plot & strand == '+', start]) - vis_win,
       visto   = max(TR.merged.data.gene[gene == gene_to_plot & strand == '+', end  ]) + vis_win,
       legend.position='right',  rel_widths=c(1, 1), title = 'NOIR')
ggsave('../EHV-1 dynamic article/Review 2024 nov/NOIR.jpg', height = 5, width = 10)
...
mapfun(gene_to_plot,  orf.sum.perc, 
       ...
       visfrom = min(TR.merged.data.gene[gene == gene_to_plot & strand == strand_to_plot, start]) - vis_win,
       visto   = max(TR.merged.data.gene[gene == gene_to_plot & strand == strand_to_plot, end  ]) + vis_win,
       legend.position='right',  rel_widths=c(1, 1), title = 'NOIR')
ggsave('../EHV-1 dynamic article/Review 2024 nov/ORF65B.jpg', height = 5, width = 10)
...
mapfun(gene_to_plot,  orf.sum.perc, 
       ...
       #visto   = max(TR.merged.data.gene[gene == gene_to_plot & strand == strand_to_plot, end  ]) + vis_win,
       legend.position='right',  rel_widths=c(1, 1), title = 'NOIR')
ggsave('../EHV-1 dynamic article/Review 2024 nov/ORF65.jpg', height = 5, width = 10)
...
Fig.isoforms1 <- cowplot::plot_grid(Fig.abunds1, Fig.annots1, ncol=2, rel_widths = c(1,1))
...
Fig.isoforms2 <- cowplot::plot_grid(Fig.abunds2, Fig.annots2, ncol=2, rel_widths = c(1,1))



Fig.isoforms <- cowplot::plot_grid(Fig.isoforms1, Fig.isoforms2, ncol=2, rel_widths = c(1,1))

ggsave('../EHV-1 dynamic article/Review 2024 nov/Figure 7_REVIEW_ori.tif', Fig.isoforms, height = 12, width = 24, dpi = 300)
ggsave('../EHV-1 dynamic article/Review 2024 nov/Figure 7_REVIEW.jpg', Fig.isoforms, height = 12, width = 24)
...
Fig.abunds1 <- cowplot::plot_grid(plotlist = list(
  ...
  mapfun('ORF65', orf.sum.perc, legend.position='right', title = 'B', what2plot = 'abund_only',
         recalc=T,
         strand_to_plot = '-',
...
Fig.abunds2 <- cowplot::plot_grid(plotlist = list(
  ...
  mapfun('ORF38', orf.sum.perc, legend.position='right', title = 'C', what2plot = 'abund_only'),
  mapfun('ORF9', orf.sum.perc, legend.position='right', title = 'D', what2plot = 'abund_only')
), ncol = 1, align = 'vh', axis = 'tlbr')
...
Fig.isoforms2 <- cowplot::plot_grid(Fig.abunds2, Fig.annots2, ncol=2, rel_widths = c(1.5, 1))


Fig.splice <- cowplot::plot_grid(Fig.isoforms1, Fig.isoforms2, ncol=2, rel_widths = c(1, 1.5))


ggsave('../EHV-1 dynamic article/Review 2024 nov/Figure 3_REVIEW_ori.tif', Fig.splice, height = 10, width = 24, dpi = 300)
ggsave('../EHV-1 dynamic article/Review 2024 nov/Figure 3_REVIEW.jpg', Fig.splice, height = 10, width = 24)
...
Fig.abunds <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF8',  orf.sum.perc, legend.position='right',  title = 'A', what2plot = 'abund_only'),
  mapfun('ORF53', orf.sum.perc, legend.position='right',  title = 'B', what2plot = 'abund_only'),
  mapfun('ORF58', orf.sum.perc, legend.position='right',  title = 'C', what2plot = 'abund_only')
), ncol =  1)


Fig.annots <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF8',  orf.sum.perc, what2plot = 'annot_only'),
  mapfun('ORF53', orf.sum.perc, what2plot = 'annot_only'),
  mapfun('ORF58', orf.sum.perc, what2plot = 'annot_only')
), ncol =  1)


Fig.splice <- cowplot::plot_grid(Fig.abunds, Fig.annots, ncol=2, rel_widths = c(1.5, 1))

ggsave('../EHV-1 dynamic article/Review 2024 nov/Supplementary Figure 3_REVIEW_ori.tif', Fig.splice, height = 12, width = 13, dpi = 300)
ggsave('../EHV-1 dynamic article/Review 2024 nov/Supplementary Figure 3_REVIEW.jpg', Fig.splice, height = 12, width = 13)
...
## Original arrangemnet

### Figure 7 Review
```{r, fig.show='hold',  fig.width = figw, fig.height = figh, eval=F}

Fig.isoforms <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF11', orf.sum.perc, legend.position='right', title = 'A'), #  axis.title.x=element_blank(), axis.title.y=element_blank(), 
  mapfun('ORF13', orf.sum.perc, legend.position='right', title = 'B'),
  mapfun('ORF14', orf.sum.perc, legend.position='right', title = 'C'),
  mapfun('ORF19', orf.sum.perc, legend.position='right', title = 'D'),
  mapfun('ORF40', orf.sum.perc, legend.position='right', title = 'E'),
  mapfun('ORF54', orf.sum.perc, legend.position='right', title = 'F')
), ncol = 2, align = 'vh', axis = 'tlbr')

#ggsave('../EHV-1 dynamic article/Figures/Fig.isoforms.small.svg', 
#       Fig.isoforms, height = 10, width = 20)

ggsave('../EHV-1 dynamic article/Review 2024 nov/Figure 7_REVIEW.tiff', 
       Fig.isoforms, height = 12, width = 24, dpi = 300)
```

### Figure 3 Review
```{r, fig.show='hold',  fig.width = figw, fig.height = figh, eval=F}

Fig.splice <- cowplot::plot_grid(plotlist = list(
  mapfun('NOIR',  orf.sum.perc, legend.position='right',  rel_widths=c(1, 1), title = 'A'),
  mapfun('ORF9',  orf.sum.perc, legend.position='right',  rel_widths=c(1, 1), title = 'B'),
  mapfun('ORF38', orf.sum.perc, legend.position='right',  rel_widths=c(1, 1), title = 'C'),
  mapfun('ORF65', orf.sum.perc, legend.position='right',  rel_widths=c(1, 1), title = 'D')
), ncol = 1, rel_heights = c(1,1,1,1))

ggsave('../EHV-1 dynamic article/Review 2024 nov/Figure 3_REVIEW.tif', 
       Fig.splice, height = 12, width = 13, dpi = 300)
```
...
Fig.splice <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF8',  orf.sum.perc, legend.position='right',  rel_widths=c(1, 1), title = 'A'),
  mapfun('ORF53', orf.sum.perc, legend.position='right',  rel_widths=c(1, 1), title = 'B'),
  mapfun('ORF58', orf.sum.perc, legend.position='right',  rel_widths=c(1, 1), title = 'C')
), ncol =  1)

ggsave('../EHV-1 dynamic article/Review 2024 nov/Supplementary Figure 3_REVIEW.tiff', 
       Fig.splice, height = 12, width = 13, dpi = 300)
...
```{r, fig.show='hold',  fig.width = figw, fig.height = figh, eval=F}
Fig.isoforms <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF11', orf.sum.perc, legend.position='none') + ggtitle('A'),
  mapfun('ORF13', orf.sum.perc, legend.position='none') + ggtitle('B'),
  mapfun('ORF14', orf.sum.perc, legend.position='none') + ggtitle('C'),
  mapfun('ORF19', orf.sum.perc, legend.position='none') + ggtitle('C'),
  mapfun('ORF40', orf.sum.perc, legend.position='none') + ggtitle('C'),
  mapfun('ORF54', orf.sum.perc, legend.position='none') + ggtitle('D')
), nrow = 2)

ggsave('../EHV-1 dynamic article/Figures/Fig.isoforms.svg', 
       Fig.isoforms, height = 20, width = 40)

```

### Figure 7
```{r, fig.show='hold',  fig.width = figw, fig.height = figh, eval=F}
#gene.sizes[1,1:14] <- gene.sizes[1,1:14] * 0.5
#sizes <- 6
#transcript.sizes
Fig.isoforms <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF11', orf.sum.perc, legend.position='right',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = 'A'),
  mapfun('ORF13', orf.sum.perc, legend.position='right',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = 'B'),
  mapfun('ORF14', orf.sum.perc, legend.position='right',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = 'C'),
  mapfun('ORF19', orf.sum.perc, legend.position='right',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = 'D'),
  mapfun('ORF40', orf.sum.perc, legend.position='right',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = 'E'),
  mapfun('ORF54', orf.sum.perc, legend.position='right',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = 'F')
), ncol = 2, align = 'vh', axis = 'tlbr')

#ggsave('../EHV-1 dynamic article/Figures/Fig.isoforms.small.svg', 
#       Fig.isoforms, height = 10, width = 20)

ggsave('../EHV-1 dynamic article/Figures/Figure 7 isoforms.tiff', 
       Fig.isoforms, height = 12, width = 21, dpi = 300)
```
...
#transcript.sizes
Fig.isoforms <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF11', orf.sum.perc, legend.position='right',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = NULL),
  mapfun('ORF13', orf.sum.perc, legend.position='right',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = NULL),
  mapfun('ORF14', orf.sum.perc, legend.position='right',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = NULL)
  #mapfun('ORF19', orf.sum.perc, legend.position='none',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = 'D'),
  #mapfun('ORF40', orf.sum.perc, legend.position='none',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = 'E'),
  #mapfun('ORF54', orf.sum.perc, legend.position='none',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = 'F')
), ncol = 1)

#ggsave('../EHV-1 dynamic article/Figures/Fig.isoforms.small.svg', 
#       Fig.isoforms, height = 10, width = 20)

ggsave('../EHV-1 dynamic article/Figures/Figure 7 isoforms.v1.tiff', 
       Fig.isoforms, height = 8, width = 7, dpi = 300)
```

###  Figure 8
```{r, fig.show='hold',  fig.width = figw, fig.height = figh, eval=F}
Fig.isoforms <- cowplot::plot_grid(plotlist = list(
  #mapfun('ORF11', orf.sum.perc, legend.position='none',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = 'A'),
  #mapfun('ORF13', orf.sum.perc, legend.position='none',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = 'B'),
  #mapfun('ORF14', orf.sum.perc, legend.position='none',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = 'C'),
  mapfun('ORF19', orf.sum.perc, legend.position='none',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = NULL),
  mapfun('ORF40', orf.sum.perc, legend.position='none',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = NULL),
  mapfun('ORF54', orf.sum.perc, legend.position='none',  axis.title.x=element_blank(), axis.title.y=element_blank(), title = NULL)
), ncol = 1)

#ggsave('../EHV-1 dynamic article/Figures/Fig.isoforms.small.svg', 
#       Fig.isoforms, height = 10, width = 20)

ggsave('../EHV-1 dynamic article/Figures/Figure 8 isoforms.v1.tiff', 
       Fig.isoforms, height = 8, width = 7, dpi = 300)
...
### Replicates with loess
```{r, fig.show='hold',  fig.width = figw, fig.height = figh, eval=F}

#DT <- orf.sum.perc

mapfun <- function(gene_to_plot, DT, ...) { 
  
    filename <- paste0(fig.dir, '/', 'IF.ratio_and_annotation_', gene_to_plot, '_', 'replicates_loess.jpg')
  
    ggrt <- plotfun(DT[gene == gene_to_plot], save.plot = F) 
    ...
    if (is.null('ggtr')) {
      ggcomb <- cowplot::plot_grid(ggrt, ggtr, nrow = 1, rel_widths = c(3, 0.5))
    } else {
      ggcomb <- cowplot::plot_grid(ggrt, ggtr, nrow = 1, rel_widths = c(3, 2))
    }
    ggsave(filename, ggcomb, width = figw, height = figh)
...
### Mean with SD with loess
```{r, fig.show='hold',  fig.width = figw, fig.height = figh, eval=F}

mapfun <- function(gene_to_plot, DT, ...) { 
  
    filename <- paste0(fig.dir, '/', 'IF.ratio_and_annotation_', gene_to_plot, '_', 'mean.SD_linear.jpg')
  
    ggrt <- plotfun(DT[gene == gene_to_plot], 
                    geom1 = geom_pointrange(aes(x=Time, y=mean, ymin = mean-sd, ymax = mean+sd, colour = Ref_Class)),
                    geom2 = geom_line(aes(x=Time, y=mean, colour = Ref_Class, group = Ref_Class)),
                    save.plot = F) 
...
mapfun <- function(gene_to_plot, DT, ...) { 
    ...
    ggsave(filename, ggrt, width = figw, height = figh)
