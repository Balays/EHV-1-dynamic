...
###
viral.ref <- "NC_001491.2.mRNA_correct_genes_with.gff3"
source('_WF.part0.R')
...
ref.TESs <- ref.TESs[!TES %in% c(11062, 51693)]
...
### Filter for those that have their 3-prime sites in the previous annotation
##tx_uni <- tx_uni[!is.na(TX_nov_ID),]
...
ggplot(tx_uni_trs_nov, ## [,.N,CAGE_cluster_width]
       aes(CAGE_cluster_width)) + 
  geom_density() +
  theme_bw() +
  labs(title = 'Density of CAGE TSS Cluster widths')
...
CAGEID <- 'CAGE_cluster_789'
tx_uni_trs_nov_mclust_all <- data.table(NULL)


for (CAGEID in unique(tx_uni_trs_nov[,CAGE_ID])) {
  
  tx_uni_trs_nov_mclust <- tx_uni_trs_nov[CAGE_ID == CAGEID, .(count=sum(sum_count)), 
                                          by=.(seqnames, strand, CAGE_ID, prime5.TR)]
  
  setnames(tx_uni_trs_nov_mclust, 'prime5.TR', 'position')
  
  if (nrow(tx_uni_trs_nov_mclust) > 5 ) {
...
ggplot(tx_uni_trs_nov_mclust_all, ## [,.N,CAGE_cluster_width]
       aes(cluster_width)) + 
  geom_density() +
  theme_bw() +
  labs(title = 'Density of CAGE TSS Cluster widths')
...
ggplot(tx_uni_trs_nov_mclust_all[,.N,cluster_width],
       aes(cluster_width, N)) + 
  geom_col() +
  scale_x_continuous(breaks = c(0,1,2,5,10,15,20,30,50)) +
  xlim(c(0,100)) +
  theme_bw() +
  labs(title = 'Density of Novel CAGE TSS Cluster widths')

tx_uni_trs_nov_mclust_all[,.N,cluster_width][order(cluster_width)]

```


```{r}
#### Use custom clustering function
CAGEID <- unique(tx_uni_trs_nov[,CAGE_ID])[]
tx_uni_trs_nov_customclust_all <- data.table(NULL)


for (CAGEID in unique(tx_uni_trs_nov[,CAGE_ID])) {
  
  tx_uni_trs_nov_customclust <- tx_uni_trs_nov[CAGE_ID == CAGEID, .(count=sum(sum_count)), 
                                          by=.(seqnames, strand, CAGE_ID, prime5.TR)]
  
  setnames(tx_uni_trs_nov_customclust, 'prime5.TR', 'position')

  
  if (nrow(tx_uni_trs_nov_customclust) > 5) {
...
c(tx_uni_trs_nov_customclust_all[CAGE_ID == 'CAGE_cluster_1897', min(cluster_start)],
  tx_uni_trs_nov_customclust_all[CAGE_ID == 'CAGE_cluster_1897', max(cluster_end)])

c(tx_uni_trs_nov[CAGE_ID == 'CAGE_cluster_1897', min(CAGE.cluster.start)],
  tx_uni_trs_nov[CAGE_ID == 'CAGE_cluster_1897', max(CAGE.cluster.end)])


ggplot(tx_uni_trs_nov_customclust_all, ## [,.N,CAGE_cluster_width]
       aes(cluster_width)) + 
  geom_density() +
  theme_bw() +
  labs(title = 'Density of CAGE TSS Cluster widths')
...
ggplot(tx_uni_trs_nov_customclust_all[,.N,cluster_width],
       aes(cluster_width, N)) + 
  geom_col() +
  scale_x_continuous(breaks = c(0,1,2,5,10,15,20,30,50)) +
  theme_bw() +
...
tx_uni_trs_nov_customclust_all[,.N,cluster_width][order(cluster_width)]
```


```{r}

## cluster all 5-prime ends

tx_uni_trs_nov_customclust_dcDNA <- 
      CustomClustering(
        tx_uni_trs_nov[, 
                       .(seqnames, strand, count=sum_count, position=prime5.TR, CAGE_ID, transcript_id)],
        max_cluster_size = 25,
        max_distance_within_cluster = 5,  # Adjust as needed
        sep_strands   = FALSE,
        include_zeros = FALSE,
...
ggplot(tx_uni_trs_nov_customclust_dcDNA[,.N,cluster_width],
       aes(cluster_width, N)) + 
  geom_col() +
  scale_x_continuous(breaks = c(0,1,2,5,10,15,20,30,50)) +
  theme_bw() +
  labs(title = 'Density of dcDNA TSS Cluster widths')

tx_uni_trs_nov_customclust_dcDNA[,.N,cluster_width][order(cluster_width)]
...
final_tx_nov_clust  <- merge(tx_uni_trs_nov, tx_uni_trs_nov_customclust_all, ## OR: tx_uni_trs_nov_mclust_all
                             by.x=c('seqnames', 'strand', 'sum_count', 'CAGE_ID', 'prime5.TR'),
                             by.y=c('seqnames', 'strand', 'count', 'CAGE_ID', 'position')
                          ) 
...
## sum counts again and add final_TX_ID based on the new 5-prime clusters

final_tx_nov_clust[, final_TX_ID := .GRP, 
                   by=.(seqnames, strand, Ref.prime3.TR, TES, CAGE_ID, TX_nov_ID, cluster_ID, cluster_peak)]

## these contain the novel TX IDs and clusters with the original TransFrags and class_code
final_tx_nov <- final_tx_nov_clust[,.(sum_count = sum(sum_count), prime5.TR = cluster_peak), 
                                   by=.(seqnames, strand, Ref.prime3.TR, TES, cmp_ref, class_code, gene,
                                        ...
                                        cluster_ID, cluster_width, final_TX_ID)]

final_tx_nov[,.(sum_class_count = sum(sum_count)), by=.(class_code)]


## this is the novel transcripts based on the the novel TX IDs
final_tx_nov_clust <- unique(final_tx_nov_clust[,.(sum_count = sum(sum_count), prime5.TR = cluster_peak), 
                                   by=.(seqnames, strand, Ref.prime3.TR, TES, 
                                        TX_nov_ID, CAGE_ID, CAGE_significance, score, support, 
                                        cluster_ID, cluster_width, final_TX_ID)])
...
```{r}
## make gff from the Novel Transcripts
final_tx_nov_gff <- final_tx_nov_clust[, .(seqnames, strand, Ref.prime3.TR, prime5.TR, final_TX_ID)]
...
final_tx_nov_filt <- final_tx_nov_clust[sum_count >= 5 &
            support > 2 &
            score > 300 &
            CAGE_significance != '*'
            ]
...
final_tx_nov_filt_gff <- as.data.table(as.data.frame(rtracklayer::import.gff3('final_tx_nov_filt.gff3')))
...
## 1.) compare to CAGE TSS

## 2.) compare to ref TES
#trids.spliced.gff
#trids.spliced
## 3.) compare to ref introns


## -->> I carried out the analysis but we decided not to include the novel splice sites
...
## 1.)
trids.cage <- TR.gff.compare.uni.CAGE.ref.counts.melt.hpi[
  transcript_id %in% trids.spliced
  #& !is.na(CAGE_significance), 
  , .(sum_count=sum(count)), 
         by=.(seqnames, strand, transcript_id, prime3.TR, prime5.TR, 
              CAGE_ID, CAGE.cluster.start, CAGE.cluster.end, CAGE_significance, support, score)]

## assign novel TX_ID, which is the same as transcript_id, as we dont want to cluster the spliced reads
trids.cage[,TX_ID := transcript_id]
...
## van egy két TES, ami túl közel van a szomszédjához -->> hiba az annotációban
ref.TESs <- ref.TESs[!TES %in% c(11062, 51693)]
...
```{r}
### merge spliced transcripts back with exon/intron data

TX_nov <- foo.filt[,.(sum_TX_count=sum(sum_count)), 
                   by=.(seqnames, strand, TX_ID, Ref.prime3.TR, TES, 
                        CAGE_ID, CAGE.cluster.start, CAGE.cluster.end, CAGE_significance, support, score)]


TX_nov <- merge(TX_nov, trids.spliced.gff[type== 'exon', .(seqnames, strand, transcript_id, start, end, exon_number, n_exon)], 
                by.x=c('seqnames', 'strand', 'TX_ID'), by.y = c('seqnames', 'strand', 'transcript_id'), all.x=T)

# exon numbers
TX_nov[, exon_number := length(unique(start)), by=.(seqnames, strand, TX_ID)]
TX_nov[, n_exon      := seq_along(exon_number), by=.(seqnames, TX_ID)]
TX_nov[, exon_number := as.integer(exon_number)]

# Calculate intron start and end
TX_nov[, intron_start := c(NA, head(end, -1) + 1), by=TX_ID]
TX_nov[, intron_end   := start - 1]
   
## intron donor and acceptor
#TX_nov[, intron_donor    := fifelse(strand == '+', intron_start, intron_end)]
#TX_nov[, intron_acceptor := fifelse(strand == '+', intron_end,   intron_start)]

# intron numbers
TX_nov[, intron_number := n_exon -1]
TX_nov[, n_intron      := exon_number -1]


### for reference also
viral.mrna[, exon_number := length(unique(start)), by=.(seqnames, strand, transcript_id)]
viral.mrna[type == 'exon', n_exon := seq_along(exon_number), by=.(seqnames, transcript_id)]
viral.mrna[, exon_number := as.integer(exon_number)]

viral.ex <- viral.mrna[type == 'exon', .(seqnames, strand, transcript_id, start, end, exon_number, n_exon)]

# Calculate intron start and end
viral.ex[, intron_start := c(NA, head(end, -1) + 1), by=transcript_id]
viral.ex[, intron_end   := start - 1]
   
## intron donor and acceptor
#viral.ex[, intron_donor    := fifelse(strand == '+', intron_start, intron_end)]
#viral.ex[, intron_acceptor := fifelse(strand == '+', intron_end,   intron_start)]

# intron numbers
viral.ex[, intron_number := n_exon -1]
viral.ex[, n_intron      := exon_number -1]


ref.introns <- unique(viral.ex[, .(seqnames, strand, intron_start, intron_end)]) ## intron_donor, intron_acceptor
ref.introns <- ref.introns[complete.cases(ref.introns)]
ref.introns[,intron_ID := paste0('intron_', seq_along(1:nrow(ref.introns)))]

## or from pre-filt LoRTIA
lortia <- as.data.table(as.data.frame(rtracklayer::import.gff('EHV_dRNA_minion_20210129_pass_mapped_intron.gff3')))


### merge together (nucleotide-precision)
TX_nov <- merge(TX_nov, ref.introns, by = c('seqnames', 'strand', 'intron_start', 'intron_end'), all.x=T) ## , 'intron_donor', 'intron_acceptor'

TX_nov[,ref.intron_nr := sum(!is.na(intron_ID)), by=TX_ID]

## intron composition
TX_nov[,intron_composition := paste(na.omit(intron_ID), collapse='::'), by=TX_ID]
#TX_nov_splice_clust <- dcast(TX_nov_splice, seqnames + strand + TX_ID + sum_TX_count + exon_number ~ n_exon, value.var = 'intron_ID')
```


```{r}
### compare with the TSSs from the previous analyis
TX_nov[,TSS := fifelse(strand == '+', min(start), max(end)), by=TX_ID]

TX_nov_TSS <- unique(TX_nov[,.(seqnames, strand, TX_ID, TSS, TES)])

TX_nov_TSS[,start := TSS][, end := TSS]

TSSs <- unique(final_tx_nov_clust[,.(seqnames, strand, TSS = prime5.TR)])
TSSs[,start := TSS - 10][, end := TSS + 10]


TX_nov_TSS_fo <- foverlaps2(TX_nov_TSS, TSSs, by=c('seqnames', 'strand', 'start', 'end'), minoverlap = 1)

setnames(TX_nov_TSS_fo, c('i.TSS', 'TSS'), c('TSS', 'Ref.TSS'))



## find the closeest
TX_nov_TSS_fo[,TSS_dist := abs(Ref.TSS - TSS)]
TX_nov_TSS_fo[,closest.TSS := min(TSS_dist), by=TX_ID]
TX_nov_TSS_fo <- TX_nov_TSS_fo[closest.TSS == TSS_dist]


TX_nov_TSS <- merge(TX_nov, 
                    TX_nov_TSS_fo[,.(seqnames, strand, TX_ID, TES, TSS, Ref.TSS)],
                    by=c('seqnames', 'strand', 'TX_ID', 'TES', 'TSS'), all.x=T)
```


```{r}
### Filtering and Refine TSSs
TX_nov_splice <- TX_nov_TSS

## overweite TSS to ref TSS
TX_nov_splice[,start := fifelse(strand == '+' & n_exon == 1, Ref.TSS, start), by=TX_ID]
TX_nov_splice[,end   := fifelse(strand == '-' & exon_number == n_exon, Ref.TSS, end), by=TX_ID]


## overweite TES to ref TES
TX_nov_splice[,end    := fifelse(strand == '+' & exon_number == n_exon, Ref.prime3.TR, end), by=TX_ID]
TX_nov_splice[,start  := fifelse(strand == '-' & n_exon == 1, Ref.prime3.TR, start), by=TX_ID]


## filter for those that had no TSS
TX_nov_splice <- TX_nov_splice[!is.na(Ref.TSS), ]
```


```{r}
## filter for those that had no ref introns
TX_nov_splice_filt <- TX_nov_splice[ref.intron_nr == n_intron & intron_composition != '', ]


### ID based on Ref.TSS, intron composition and Ref.TES
TX_nov_splice_filt[,TX_nov_ID := .GRP, by=.(seqnames, strand, CAGE_ID, Ref.TSS, intron_composition, Ref.prime3.TR, TES)]
TX_nov_splice_filt[,sum_TX_count := sum(sum_TX_count), by=TX_nov_ID]

```


```{r}
TX_nov_splice_clust <- unique(TX_nov_splice_filt[,.(seqnames, strand, TX_nov_ID, start, end, Ref.TSS, Ref.prime3.TR, TES, CAGE_ID, intron_composition, sum_TX_count)])

TX_nov_splice_clust[,.N,sum_TX_count][order(sum_TX_count)]
ggplot(TX_nov_splice_clust) + 
  geom_density(aes(sum_TX_count))
...
## filter
TX_nov_splice_clust_filt <- TX_nov_splice_clust[sum_TX_count >= 5]

```


```{r}
## make gff from the Novel Spliced Transcripts
final_tx_nov_splice_gff_ex <- unique(TX_nov_splice_clust_filt[, .(seqnames, strand, type='exon', 
                                                        Parent = TX_nov_ID, 
                                                        ID = TX_nov_ID, 
                                                        transcript_id = TX_nov_ID, 
                                                        start, end, Ref.TSS, Ref.prime3.TR, TES, CAGE_ID, TX_nov_ID, score=sum_TX_count, intron_composition)])

final_tx_nov_splice_gff_tr <- unique(TX_nov_splice_clust_filt[, .(seqnames, strand, type='transcript', 
                                                        ID = TX_nov_ID, 
                                                        transcript_id = TX_nov_ID, 
                                                        start = min(start), end=max(end), Ref.TSS, Ref.prime3.TR, TES, CAGE_ID, score=sum_TX_count, intron_composition), by=TX_nov_ID])


final_tx_nov_splice_gff <- rbind(final_tx_nov_splice_gff_ex, 
                                 final_tx_nov_splice_gff_tr,
                                 fill=TRUE)


final_tx_nov_splice_gff[, ':=' (source = 'Tombácz_et_al', phase=NA), ]


rtracklayer::export.gff3(as.data.frame(final_tx_nov_splice_gff), 'final_tx_nov_splice.gff3')
...
```{r}
## filter for those that had no ref introns
TX_nov_splice_filt <- TX_nov_splice[intron_composition == '', ]

## novel intron ids
TX_nov_introns <- unique(TX_nov_splice_filt[,.(seqnames, strand, intron_start, intron_end)])[complete.cases(intron_start)]
TX_nov_introns[,nov.intron_ID := paste0('intron_', seq(1, nrow(TX_nov_introns)))]

TX_nov_splice_filt <- merge(TX_nov_splice_filt, TX_nov_introns, by=c('seqnames', 'strand', 'intron_start', 'intron_end'), all.x=T)
TX_nov_splice_filt[,intron_composition := paste(na.omit(nov.intron_ID), collapse = ';'), by=TX_ID]
...
## filter
TX_nov_splice_clust_filt <- TX_nov_splice_clust[sum_TX_count >= 3]
...
final_tx_nov_splice_gff_nov <- rbind(final_tx_nov_splice_gff_ex, 
                                 final_tx_nov_splice_gff_tr,
                                 fill=TRUE)


final_tx_nov_splice_gff_nov[, ':=' (source = 'Tombácz_et_al', phase=NA), ]


rtracklayer::export.gff3(as.data.frame(final_tx_nov_splice_gff_nov), 'final_tx_nov_splice_nov.gff3')

final_tx_nov_splice_gff_nov <-  as.data.table(as.data.frame(rtracklayer::import.gff3('final_tx_nov_splice_nov.gff3')))
...
### Export
```{r}
fwrite(data.table(transcript_id=trids), 'TR_IDs_nov.tsv', sep='\t')
fwrite(final_tx_nov_clust, 'final_tx_nov_clust.tsv', sep='\t')
```
