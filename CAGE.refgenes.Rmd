...
```{r, eval=F}
## make transcritps from these
TX_nov_splice_clust <- unique(TX_nov_splice[!is.na(intron_ID), .(seqnames, strand, TX_ID, sum_TX_count, intron_composition, intron_ID, intron_number, n_intron)])


TX_nov_splice_clust <- merge(TX_nov_splice_clust, ref.introns, by=c('seqnames', 'strand', 'intron_ID'), all.x=T)


TX_nov_splice_clust <- merge(TX_nov_splice_clust,
                             unique(TX_nov_splice[, .(seqnames, strand, TX_ID, start, end, exon_number, n_exon, intron_number)]),
                             by=c('seqnames', 'strand', 'TX_ID', 'intron_number'),
                             all=T)

TX_nov_splice_clust <- merge(TX_nov_splice_clust,
                             unique(TX_nov_splice[, .(seqnames, strand, TX_ID, Ref.TSS, CAGE_ID, Ref.prime3.TR, TES)]),
                             by=c('seqnames', 'strand', 'TX_ID'),
                             all.x=T)
```
...
rtracklayer::export.gff3(as.data.frame(final_tx_nov_splice_gff), 'final_tx_nov_splice_nov.gff3')
