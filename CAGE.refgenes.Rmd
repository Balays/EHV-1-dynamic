...
tx_uni_trs <- 
  TR.gff.compare.uni.CAGE.ref.counts.melt.hpi.TXnov[,.(sum_count=sum(count)),
        by=.(seqnames, strand, prime3.TR, prime5.TR, transcript_id,
             CAGE_ID, CAGE_significance, score, support, TX_ID, TX_nov_ID, Ref.prime3.TR, TES,
...
final_tx_nov_filt <- final_tx_nov[sum_count >= 20 &
            support > 2 &
            score > 300 &
            CAGE_significance != '*'
            ]

length(unique(final_tx_nov_filt[,transcript_id]))


final_tx_nov_filt[,.N,gene]
```


```{r}
## make gff from them
final_tx_nov_filt_gff <- final_tx_nov_filt[,
                                           .(seqnames, strand, type = 'transcript', Ref.prime3.TR, prime5.TR, TX_nov_ID)]

final_tx_nov_filt_gff[, start := fifelse(strand == '+', prime5.TR, Ref.prime3.TR)]
final_tx_nov_filt_gff[, end   := fifelse(strand == '+', Ref.prime3.TR, prime5.TR)]
final_tx_nov_filt_gff[, ':=' (source = 'Tombácz_et_al', width = end-start+1, phase=NA, score=NA), ]
