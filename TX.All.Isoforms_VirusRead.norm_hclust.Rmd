---
title: "EHV-1 Gene dynamics (All Trancsripts)"
author: Balázs Kakuk
output: 
  html_document:
    theme: cosmo
...
# Gene dynamics (all TX isoforms)

## Those reads were counted only, where *both* the *3-prime* AND the *5-prime* adapters were "correct" (according to LoRTIA)

## The gene counts originate from gff-compare transcript counting
## The gene annotation for each transcript was assigned by Torma et al
## Only the previoulsly annotated Transcripts and the now described novel ones were counted (as supported by the CAGE)
...
```{r}
## Import TX-counting-based gene counts

orf.sum.gene.TX <- fread("LoRTIA_virus/TX.ratio/TX.ratio_gene_counts.tsv")
orf.sum         <- orf.sum.gene.TX
  
ORFs <- unique(c(ORFs, unique(na.omit(orf.sum$gene_cluster)), unique(na.omit(orf.sum$gene))))

orf.sum[, gene          := factor(gene, levels=ORFs)]
orf.sum[, gene_cluster  := factor(gene_cluster, levels=ORFs)]
```
