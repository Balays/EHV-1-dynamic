...
unique(TR.merged.data[,.(transcript_id, strand, CAGE_significance)])[,.N,CAGE_significance]

TR.merged.data.CAGE <- copy(TR.merged.data)
```
...
```{r}
TR.merged.data.CAGE.merge <- 
  merge(unique(TR.merged.data[,.(transcript_id, start.TR, end.TR, strand, CAGE_significance, CAGE_ID)]), 
      unique(TR.merged.data.CAGE[,.(transcript_id, start.TR, end.TR, strand, CAGE_significance, CAGE_ID)]), 
      by=c('transcript_id', 'strand') )


TR.merged.data.CAGE.merge[is.na(CAGE_significance.x), CAGE_significance.x := '']

TR.merged.data.CAGE.merge[start.TR.x != start.TR.y, ]
TR.merged.data.CAGE.merge[end.TR.x != end.TR.y, ]

TR.merged.data.CAGE.merge[CAGE_significance.x != CAGE_significance.y, ]
transcript_id_sus <- TR.merged.data.CAGE.merge[CAGE_significance.x != CAGE_significance.y, ]
...
TR.all.data.sp[,.N,by=.(source,TX_category)]
...
cowplot::plot_grid(gg1, gg2, gg3, gg4, nrow = 2) + ggtitle('Tombácz et al transcripts')


ggsave('Tombácz et al transcripts.venn.jpg', width=12, height=7)
```
...
putatives_to_keep <- unique(
    TR.all.data.sp[  presence >= 3 & sum_dcDNA_read_count >= 3
                   & CAGE_significance %in% c('*', '**', '***') 
                   & TR.all.data.sp$CAGE_TSS_ratio >= CAGE_TSS_ratio_thresh 
                   & NAGATA.support %in% c('distance <= 25')     
                   #& source == 'Tombacz_et_al' & 
                   & TX_category == 'Putative mRNA', transcript_id]
)
...
TRs.to.keep <- unique(c(all_non_putative, putatives_to_keep))
...
ann.plotfun <- function(gene_to_plot=NULL, plot.data=plot.data, 
  ...
  ggann <- plot.genome.region(visfrom = visfrom, visto = visto, 
             ...
             sizes=10, gene.sizes=gene.sizes, gene.label.col='black', alpha=alpha, #plot.title=NA, 
             palette=palette[1:3], tr.palette = tr.palette, # palette[c(15,18)],
             tolower=F, vline=NULL, vline2=NULL, breakseq=breakseq, margins=NULL, #unit(c(1,1,1,1), "cm"),
             scales=scales, genomplot.scale=genomplot.scale, ylim=c(0, NA), y.log10=F, y.log2=F, ylims.gene=ylims.gene, y.multip=1.5, 
             ...
             labels=NULL, legend.position.prime = 'top', theme_general = theme_general, 
             return.plot.data=F
             , ...
             )
...
plotfun <- function(DT, 
  ...
  if (nrow(DT) == 0) { 
    ...
    ggp <- 
      ...
      guides(fill = guide_legend(nrow = legend.nrow, ncol = legend.ncol, byrow=TRUE)) +
...
mapfun <- function(gene_to_plot=NULL,  DT=orf.sum.perc, TRs_to_plot=NULL, recalc = F,
    ...
    try({
      ...
      ggtr <- ann.plotfun(gene_to_plot, vis_win=vis_win, 
                          ...
                          add.TR.labels = T, breakseq=breakseq, plot.title=plot.title)
    })
...
### Supp Fig S12 - EHV raRNAs
```{r, fig.show='hold',  fig.width = figw, fig.height = figh, eval=T}

dir.create('Figures')

Ori.df        <- gff[gff$type == 'stem_loop', c("seqnames", "start", "end", "strand", "type", "Note")]
Ori.df$Name   <- gsub(' palindrome', '', unlist(Ori.df$Note))
colnames(Ori.df)[6] <- 'gene_name'
Ori.df$gene   <- Ori.df$Name; Ori.df$ID   <- Ori.df$Name; Ori.df$gene_name <- Ori.df$Name
Ori.df$part   <- 1
Ori.df <- as.data.frame(Ori.df)
Ori.df <- Ori.df[,colnames(feature.df)]

Ori.df <- make.genome.plot.data(Ori.df, feature.col = 8, feature.name = 'ID')
Ori.df$strand <- factor(Ori.df$strand, levels = c('+', '-', '*'))

gene.ori.plotdata <- rbind(gene.plotdata, Ori.df)


## Add colors for those TR-s that overlap with the Ori (without strandness)
TR.merged.data.ori <- copy(TR.merged.data)

TR.merged.data.ori <- foverlaps2(TR.merged.data.ori, data.table(Ori.df), minoverlap = 1,
                                 by=c('seqnames',  'start', 'end'))
Ori.TRs <- unique(TR.merged.data.ori$transcript_id)

TR.merged.data.ori <- copy(TR.merged.data[,
          .(seqnames, strand, start, end, transcript_id,
            start.TR, end.TR, prime5.TR, prime3.TR,
            start.exon, end.exon, exon_number, last_exon)])

TR.merged.data.ori[,ori_overlap := strand]
TR.merged.data.ori[transcript_id %in% Ori.TRs, ori_overlap := '*']
TR.merged.data.ori[, CAGE_significance := '']



Fig.all.Ori <- cowplot::plot_grid(plotlist = list(
  ann.plotfun(NULL, vis_win=NULL, genome='NC_001491.2',
              gene.plotdata = gene.ori.plotdata,
              genomplot.scale = 12,
              visfrom = 66000, visto = 85000,
              tr.palette = colorvec[-c(3)],
              TR.merged.data = TR.merged.data.ori,
              annot_fill_column = 'ori_overlap',
              add.TR.labels = T, breakseq=breakseq, plot.title='OriL'),

  ann.plotfun(NULL, vis_win=NULL, genome='NC_001491.2',
              gene.plotdata = gene.ori.plotdata,
              genomplot.scale = 7,
              visfrom = 118000, visto = 124000,
              tr.palette = colorvec[-c(3)],
              TR.merged.data = TR.merged.data.ori,
              annot_fill_column = 'ori_overlap',
              add.TR.labels = T, breakseq=breakseq, plot.title='OriS')

), ncol = 1, align = 'v', axis = 'tlbr', rel_heights = c(3,1.4))



ggsave('./Figures/Supp Fig S12.pdf', Fig.all.Ori, height = 16, width = 10)




TR.merged.data.oriL.ov <- foverlaps2(TR.merged.data.ori, 
                                    visregion = data.table(seqnames = 'NC_001491.2', start = 66000, end = 85000), 
                                    minoverlap = 1, by=c('seqnames',  'start', 'end'))

unique(TR.merged.data.oriL.ov[grepl('novTX', transcript_id), transcript_id])



TR.merged.data.oriS.ov <- foverlaps2(TR.merged.data.ori, 
                                    data.table(seqnames = 'NC_001491.2', start = 118000, end = 124000), 
                                    minoverlap = 1, by=c('seqnames',  'start', 'end'))

unique(TR.merged.data.oriS.ov[grepl('novTX', transcript_id), transcript_id])

#Fig.all.Ori
```


### Supp Fig S13 - PRV raRNAs
```{r, fig.show='hold',  fig.width = figw, fig.height = figh, eval=T}

dir.create('Figures')

### PRV Oris
gff.file <- paste0('LT934125.1.gff3')
gff      <- data.table(as.data.frame(rtracklayer::import.gff(gff.file)))

Ori.df        <- gff[gff$type == 'stem_loop', c("seqnames", "start", "end", "strand", "type", "ID")]
Ori.df$Name   <- Ori.df$ID
colnames(Ori.df)[6] <- 'gene_name'
Ori.df$gene   <- Ori.df$Name; Ori.df$ID   <- Ori.df$Name; Ori.df$gene_name <- Ori.df$Name
Ori.df$part   <- 1
Ori.df <- as.data.frame(Ori.df)
...
### PRV Genes
feature.df.PRV <- fread('LT934125.1_feature.df.tsv')
genome.plotdata.PRV <- make.genome.plot.data(feature.df.PRV, feature.col = 8, feature.name = 'ID')
genome.plotdata.PRV$strand <- factor(genome.plotdata.PRV$strand, levels = c('+', '-', '*'))

### Combine
genome.plotdata.PRV <- rbind(genome.plotdata.PRV, Ori.df)

genome.plotdata.PRV[is.na(genome.plotdata.PRV$part), 'part'] <- 1

### Transcripts
### PRV Oris
gff.file <- paste0('LT934125.1_Torma_et_al_corrected.gff3')
gff      <- data.table(as.data.frame(rtracklayer::import.gff(gff.file)))

# 1. Subset to exons and extract the transcript id from Parent
exons <- gff[type == "exon", ]
#exons[, transcript_id := unlist(Parent)]  # Parent is a list; here we assume it holds the transcript id

# 2. For each transcript, calculate transcript boundaries
exons[, `:=`(
  start.TR = min(start),
  end.TR   = max(end)
), by = transcript_id]
...
# 3. Define prime ends: for '+' strand, 5' = transcript start; for '-' strand, 5' = transcript end.
exons[, `:=`(
  prime5.TR = ifelse(strand == "+", start.TR, end.TR),
  prime3.TR = ifelse(strand == "+", end.TR, start.TR)
)]

# 4. Number the exons in transcript order.
# Since all exons in a transcript share the same strand, we can do:
exons[, exon_number := if (unique(strand) == "+") {
    seq_len(.N)
  } else { 
    rev(seq_len(.N))
  }
...
# 5. Flag the last exon in each transcript
exons[, last_exon := fifelse(exon_number == max(exon_number), T, F), by = transcript_id]

# 6. (Optionally) rename the exon start and end columns for clarity.
exons[, `:=`(
  start.exon = start,
  end.exon   = end
)]
...
# 7. Finally, select the desired columns:
TR.merged.data.PRV <- exons[, .(seqnames, strand, transcript_id,
                            start.TR, end.TR, prime5.TR, prime3.TR,
                            start.exon, end.exon, exon_number, last_exon)]
...
# 8. Check which TR overlaps with the oriS and oriL (on both strands)
TR.merged.data.PRV.ori <- foverlaps2(TR.merged.data.PRV, data.table(Ori.df), minoverlap = 1,
                                     by=c('seqnames',  'start', 'end'))

Ori.TRs <- unique(TR.merged.data.PRV.ori$transcript_id)

TR.merged.data.PRV.ori <- copy(TR.merged.data.PRV)

# 9. Annotate the overlapping TRs
TR.merged.data.PRV.ori[,ori_overlap := strand]
TR.merged.data.PRV.ori[transcript_id %in% Ori.TRs, ori_overlap := '*']

# 10. Omit CAGE sign. asterisks
TR.merged.data.PRV.ori[, CAGE_significance := '']


### Plot

Fig.PRV.Ori <- cowplot::plot_grid(plotlist = list(
  ann.plotfun(NULL, vis_win=NULL, genome='LT934125.1',
              gene.plotdata = genome.plotdata.PRV,
              genomplot.scale = 8,
              visfrom = 55000, visto = 73500,
              tr.palette = colorvec[-c(3)],
              theme_general = theme_ipsum(),
              #theme_general = theme_bw(), #ipsum(),
              TR.merged.data = TR.merged.data.PRV.ori,
              annot_fill_column = 'ori_overlap',
              add.TR.labels = T, breakseq=breakseq, plot.title='OriL'),
...
), ncol = 1, align = 'v', axis = 'tlbr', rel_heights = c(1,1))



ggsave('./Figures/Supp Fig S13.tiff', Fig.PRV.Ori, height = 18, width = 10)
...
### Supp Fig S7 - Anomalous transcripts
...
Fig.isoforms2 <- cowplot::plot_grid(Fig.abunds2, Fig.annots2, ncol=2, rel_widths = c(1, 0.75), align = 'vh', axis = 'l')


Fig.isoforms <- cowplot::plot_grid(Fig.isoforms1, Fig.isoforms2, ncol=2, rel_widths = c(1, 1))

##
##

Fig.abunds <- cowplot::plot_grid(Fig.abunds1, Fig.abunds2, ncol=1, rel_heights = c(1), align = 'vh', axis = 'b')

Fig.annots <- cowplot::plot_grid(Fig.annots1, Fig.annots2, ncol=1, rel_heights = c(1), align = 'vh', axis = 'b')

Fig.isoforms <- cowplot::plot_grid(Fig.abunds, Fig.annots, ncol=2, rel_widths = c(1, 0.8))


#ggsave('./Figures/Figure 7_REVIEW_ori.tif', Fig.isoforms, height = 12, width = 24, dpi = 300)
ggsave('./Figures/Supp Fig S7.jpg', Fig.isoforms, height = 20, width = 12)

#Fig.isoforms
```

### Figure 6 - DTU transcripts --\>\> OK
...
Fig.abunds1 <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF19', orf.sum.perc, legend.position='bottom', what2plot = 'abund_only', title = 'A',
         recalc=F,
         strand_to_plot = '+',
  ...
  mapfun('ORF13', orf.sum.perc, legend.position='bottom', title = 'C', what2plot = 'abund_only', legend.nrow=2,
         recalc=F,
         strand_to_plot = '+',
         breakseq = 1500,
...
), ncol = 1, align = 'v', axis = 'tlbr', rel_heights = c(1, 0.65))
...
Fig.annots1 <- cowplot::plot_grid(plotlist = list(
  ...
  mapfun('ORF13', orf.sum.perc, what2plot = 'annot_only',
         ...
         visfrom  = 14980,    # min(TR.merged.data.gene[gene == 'ORF13' & strand == '+', start]) - vis_win,
         visto    = 21600 )  # max(TR.merged.data.gene[gene == 'ORF13' & strand == '+', end  ]) + vis_win),
 ), ncol = 1, align = 'v', axis = 'tlbr', rel_heights = c(0.65))


Fig.isoforms1 <- cowplot::plot_grid(Fig.abunds1, Fig.annots1, ncol=2, rel_widths = c(1, 0.6), align = 'vh', axis = 'l')



Fig.abunds2 <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF40', orf.sum.perc, legend.position='bottom', title = 'B', what2plot = 'abund_only', legend.nrow=2,
         recalc=F,
         strand_to_plot = '-',
  ...
  mapfun('ORF54', orf.sum.perc, legend.position='bottom', title = 'E', what2plot = 'abund_only', legend.nrow=3,
         recalc=F,
         strand_to_plot = '+',
...
Fig.isoforms <- cowplot::plot_grid(Fig.isoforms1, Fig.isoforms2, ncol=2, rel_widths = c(1, 0.8))

#ggsave('./Figures/Figure 7_REVIEW_ori.tif', Fig.isoforms, height = 12, width = 24, dpi = 300)
ggsave('./Figures/Figure 6.jpg', Fig.isoforms, height = 12, width = 25)

#Fig.isoforms
```

### Figure 4 - Spliced Transcripts --\>\> OK
...
Fig.abunds2 <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF8', orf.sum.perc, legend.position='right', title = 'B', what2plot = 'abund_only',
         ...
         recalc=T,
         strand_to_plot = '+',
         visfrom = 6100, #min(TR.merged.data.gene[gene == 'ORF9' & strand == '-', start]) - vis_win,
...
ggsave('./Figures/Figure 4.jpg', Fig.splice, height = 14, width = 24)

#Fig.splice
```

### Supp Figure S9 - Spliced Transcripts 2 AND DTU 2 --\>\> OK

ORF47-44, ORF65

```{r, fig.show='hold',  fig.width = figw, fig.height = figh, eval=T}
Fig.abunds1 <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF65',  orf.sum.perc, legend.position='right', what2plot = 'abund_only', title = 'A',#legend.nrow=10, legend.ncol=2,
         breakseq=1000,
         recalc=T,
         strand_to_plot = '-'
  ...
  mapfun(c('ORF44'), orf.sum.perc, legend.position='right', what2plot = 'abund_only', title = 'B',
         breakseq=1500,
         recalc=T, #min_TIF_ratio = 0.01
         #strand_to_plot = '-',
  ...
  mapfun(c('ORF53'), orf.sum.perc, legend.position='right', what2plot = 'abund_only', title = 'C',
         breakseq=1500,
         recalc=F,
         #strand_to_plot = '+',
         visfrom = 92600,
...
#Fig.abunds2 <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF58', orf.sum.perc, legend.position='right', what2plot = 'abund_only', title = 'D',
         breakseq=1500,
         recalc=T,
         strand_to_plot = '+',
         visfrom = 96600,
  ...
  mapfun('ORF14', orf.sum.perc, legend.position='right', what2plot = 'abund_only', title = 'E',
         recalc=F,
         strand_to_plot = '+',
         breakseq = 1500,
         visfrom  = 14980,    # min(TR.merged.data.gene[gene == 'ORF13' & strand == '+', start]) - vis_win,
         visto    = 21600 ),   # max(TR.merged.data.gene[gene == 'ORF13' & strand == '+', end  ]) + vis_win),
  mapfun('ORF11', orf.sum.perc, legend.position='bottom', title = 'F', what2plot = 'abund_only', legend.nrow=2,
         recalc=F,
         strand_to_plot = '+',
...
), ncol = 1, align = 'vh', axis = 'tlbr', rel_heights = c(1, 1, 1, 1))
...
), ncol = 1, align = 'vh', axis = 'tlbr', rel_heights = c(1, 1, 1, 1, 1, 1))


#Fig.isoforms1 <- cowplot::plot_grid(Fig.annots1, Fig.abunds1, ncol=2, rel_widths = c(1,2))

#Fig.isoforms2 <- cowplot::plot_grid(Fig.annots2, Fig.abunds2, ncol=2, rel_widths = c(1,2), align = 'vh', axis = 'l')

#Fig.others <- cowplot::plot_grid(Fig.isoforms1, Fig.isoforms2, ncol=1, rel_heights = c(1, 1))

Fig.others <- cowplot::plot_grid(Fig.annots1, Fig.abunds1, ncol=2, rel_widths = c(1, 2), align = 'vh', axis = 'tlbr')

#ggsave('./Figures/Figure 3_REVIEW_ori.tif', Fig.splice, height = 10, width = 24, dpi = 300)
ggsave('./Figures/Supp Fig S9.jpg', Fig.others, height = 30, width = 20)
#ggsave('./Figures/Supp Fig S9.tif', Fig.others, height = 30, width = 20)
...
Fig.abunds1 <- cowplot::plot_grid(plotlist = list(
  mapfun('NOIR', orf.sum.perc, legend.position='right', title = 'A', what2plot = 'abund_only', legend.nrow=2, legend.ncol=1,
