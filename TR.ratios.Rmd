...
mapfun <- function(gene_to_plot, DT=orf.sum.perc, recalc = F,
    ...
    try({
      ...
      TR.annot.data.filt <- TR.annot.data.filt[strand %in% strand_to_plot]
...
mapfun(gene_to_plot,  orf.sum.perc, 
       ...
       legend.position='right',  rel_widths=c(1, 1), title = 'ORF11', what2plot = 'annot_only')
...
mapfun(gene_to_plot,  orf.sum.perc, 
       ...
       #visto   = max(TR.merged.data.gene[gene == gene_to_plot & strand == '+', end  ]) + vis_win,
       visfrom =NULL, visto =NULL,
       legend.position='right',  rel_widths=c(1, 1), title = 'ORF11', what2plot = 'abund_only')
