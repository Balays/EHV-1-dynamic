...
ann.plotfun <- function(gene_to_plot=NULL, plot.data=plot.data, 
  ...
  ggann <- plot.genome.region(visfrom = visfrom, visto = visto, 
             ...
             gene.label=T, add.unstranded=T, add.feature=T, gene.feature.width.thresh=700, flip.gene.y=T, force.gene.y=T,
...
mapfun <- function(gene_to_plot=NULL,  DT=orf.sum.perc, TRs_to_plot=NULL, recalc = F,
    ...
    ggrt  <- plotfun(DTsub, 
                    ...
                    save.plot = F, ...) 
    if (is.null(gene_to_plot) & !is.null(TRs_to_plot)) { ggrt <- ggrt + facet_grid(cols=vars(gene), scales='fixed') }
...
Fig.test <- cowplot::plot_grid(
 ...
 mapfun(gene_to_plot=NULL,  orf.sum.perc, TRs_to_plot=TRs_to_plot,
       ...
       visfrom =visfrom, visto =visto,
       legend.position='right',  rel_widths=c(1, 1), title = gene_to_plot, what2plot = 'annot_only'),
  ncol=2, rel_widths = c(2, 0.65), align = 'vh', axis = 'l')
...
ORFs_toplot <- c("ORF38", "ORF45", "ORF50", "ORF54", "ORF67", "ORF70")


dir.create('Figures')

Fig.abunds1 <- cowplot::plot_grid(plotlist = list(
  mapfun(ORFs_toplot[1], orf.sum.perc, legend.position='right', title = 'A', what2plot = 'abund_only',
         recalc=F,
         strand_to_plot = '+',
         visfrom = min(TR.merged.data.gene[gene == ORFs_toplot[1] & strand == '+', start]) - vis_win,
         visto   = max(TR.merged.data.gene[gene == ORFs_toplot[1] & strand == '+', end  ]) + vis_win), 
  mapfun(ORFs_toplot[2], orf.sum.perc, legend.position='right', title = 'B', what2plot = 'abund_only',
         recalc=F,
         strand_to_plot = '+',
         visfrom = min(TR.merged.data.gene[gene == ORFs_toplot[2] & strand == '+', start]) - vis_win,
         visto   = max(TR.merged.data.gene[gene == ORFs_toplot[2] & strand == '+', end  ]) + vis_win)
  
), ncol = 1, align = 'v', axis = 'tlbr')


Fig.annots1 <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF45', orf.sum.perc, what2plot = 'annot_only',
         recalc=F,
         strand_to_plot = '+',
...
plot_data <- ggplot_build(Fig.annots1)
str(plot_data$data)
...
Fig.abunds2 <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF70', orf.sum.perc, legend.position='right', title = 'D', what2plot = 'abund_only',
         recalc=F,
         strand_to_plot = '+',
         visfrom = min(TR.merged.data.gene[gene == 'ORF70' & strand == '+', start]) - vis_win,
         visto   = max(TR.merged.data.gene[gene == 'ORF70' & strand == '+', end  ]) + vis_win),
  mapfun('ORF67', orf.sum.perc, legend.position='right', title = 'E', what2plot = 'abund_only',
         recalc=F,
         strand_to_plot = '-',
...
Fig.annots2 <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF70', orf.sum.perc, what2plot = 'annot_only',
         recalc=F,
         strand_to_plot = '+',
         visfrom = min(TR.merged.data.gene[gene == 'ORF70' & strand == '+', start]) - vis_win,
         visto   = max(TR.merged.data.gene[gene == 'ORF70' & strand == '+', end  ]) + vis_win),
  mapfun('ORF67', orf.sum.perc, what2plot = 'annot_only',
         recalc=F,
         strand_to_plot = '-',
         visfrom = min(TR.merged.data.gene[gene == 'ORF67' & strand == '-', start]) - vis_win,
         visto   = max(TR.merged.data.gene[gene == 'ORF67' & strand == '-', end  ]) + vis_win)
...
ggsave('./Figures/Figure X_REVIEW.jpg', Fig.isoforms, height = 12, width = 24)
