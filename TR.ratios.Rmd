...
mapfun <- function(gene_to_plot=NULL,  DT=orf.sum.perc, TRs_to_plot=NULL, recalc = F,
    ...
    ### use the gene-normalized transcript isoform ratios, calculated previously
    ...
    try({
      ...
      if(!is.null(gene_to_plot) & is.null(TRs_to_plot)) {
        ...
        #TR.annot.data.filt$Ref_Class[!TR.annot.data.filt$gene %in% gene_to_plot] <- NA
...
```{r, fig.show='hold',  fig.width = figw, fig.height = figh, eval=T}
ORFs_toplot <- c("ORF38", "ORF45", "ORF50", "ORF54", "ORF67", "ORF70", "ORF64")
...
Fig.abunds1 <- cowplot::plot_grid(plotlist = list(
  mapfun('ORF38', orf.sum.perc, legend.position='right', title = 'A', what2plot = 'abund_only',
         recalc=F,
         strand_to_plot = '+',
         visfrom = min(TR.merged.data.gene[gene == 'ORF38' & strand == '+', start]) - vis_win,
         visto   = max(TR.merged.data.gene[gene == 'ORF38' & strand == '+', end  ]) + vis_win), 
  mapfun('ORF45', orf.sum.perc, legend.position='right', title = 'B', what2plot = 'abund_only',
         recalc=F,
         strand_to_plot = '+',
  ...
  mapfun('ORF50', orf.sum.perc, legend.position='right', title = 'C', what2plot = 'abund_only',
         recalc=F,
         strand_to_plot = '+',
...
Fig.abunds2 <- cowplot::plot_grid(plotlist = list(
  ...
  mapfun(c('ORF67', 'ORF68'), orf.sum.perc, legend.position='right', title = 'E', what2plot = 'abund_only',
         recalc=T,
         strand_to_plot = '-',
  ...
  mapfun('ORF64', orf.sum.perc, legend.position='right', title = 'F', what2plot = 'abund_only',
         recalc=F,
         strand_to_plot = '+',
         visfrom = min(TR.merged.data.gene[gene == 'ORF64' & strand == '+', start]) - vis_win,
         visto   = max(TR.merged.data.gene[gene == 'ORF64' & strand == '+', end  ]) + vis_win)
...
ggsave('./Figures/Figure X_Anomaluous transcripts.jpg', Fig.isoforms, height = 12, width = 24)
